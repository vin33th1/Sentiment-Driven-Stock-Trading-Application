...
# Copy of Main file for testing purpposes
# Edit to tune to your needs

# all api keys and information
#Alpaca
...
def get_sentiments(list1: list, list2: list):
    chat_completion = client.chat.completions.create(
    messages=[
        {"role": "system", "content": "You are an AI language model trained to analyze and detect the sentiment of news summaries."},
        {"role": "user","content": f"Use sentiment analysis and determine if these pieces of content in the two python lists are postive negative or neutral {list1} {list2} answer with an integer value ranging from [-1,1] -1 being negative and 1 being postivie only output the numerical values as a python list for each given list",}
    ],
    model="gpt-3.5-turbo",
    )
...
stock1_summary = get_summaries('AAPL')
stock2_summary = get_summaries('NVDA')

stock_sentiments = get_sentiments(stock1_summary, stock2_summary)
